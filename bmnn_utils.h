#ifndef WHALE_CV_SDK_MODEL_BMNN2_UTILS_H
#define WHALE_CV_SDK_MODEL_BMNN2_UTILS_H
...
public:
    ...
    BMNNNetwork(void *bmrt, const std::string &name) : m_bmrt(bmrt)
    {
        ...
        if (m_mapInputs.find(channel_id) == m_mapInputs.end())
        {
            m_mapInputs.insert(make_pair(channel_id, m_inputTensors));
        }
        ...
        for (int i = 0; i < m_netinfo->output_num; ++i)
        {
            ...
            if (BM_FLOAT32 == m_netinfo->output_dtypes[i])
                max_size *= 4;
            auto ret = bm_malloc_device_byte(m_handle, &m_outputTensors[i].device_mem, max_size);
            assert(BM_SUCCESS == ret);
        }

        if (m_mapOutputs.find(channel_id) == m_mapOutputs.end())
        {
            m_mapOutputs.insert(make_pair(channel_id, m_outputTensors));
        }
    ...
    int outputTensorStageNum()
    {
        return m_netinfo->stage_num;
    }
    ...
    {
        assert(index < m_netinfo->output_num);
        return m_netinfo->output_names[index];
    }
    ...
    int forward()
    {
        ...
        if (!ok)
        {
            std::cout << "bm_launch_tensor: Failed to launch network, net_name:" << m_netinfo->name << std::endl;
            return -1;
        }
